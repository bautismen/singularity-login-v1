...
(s.idService === info.idService && s.idServiceItem === info.item)));  
    ...
    if (infoControl.currentIndex < infoControl.serviceDetail.length - 1) {
      onUpdateFormData(prev=>({
        ...
        Services: prev.Services.map((ser) => 
          ser.idControl === infoControl?.idControl && 
          ser.idService === infoControl?.idService &&
  ...
  const duplicateCard = (idServiceItem, detail) => {
    ...
    onDuplicateDetail(infoControl?.idControl, infoControl?.idService, idServiceItem, newDetail);
  };
              ...
              <div id="pageCounter">
                <div className={styles.serviceActions}>
                  ...
                  <button
                    ...
                    // disabled={isDisabled}
                    onClick={() => {
                      onRemoveDetail(infoControl?.idControl, infoControl?.idService, infoControl?.idServiceItem, detail.idDetail )
                      prevPage();
                    }}
                  >
                    <X size={18} />
