...
export const AirFreightForm: React.FC<AirFreightFormProps> = ({
    ...
    if (airServiceControl.currentIndex < airServiceControl?.serviceDetail.length - 1) {
      onUpdateFormData(prev=>({
        ...
        Services: prev.Services.map((ser) => 
          ser.idControl === airServiceControl?.idControl && 
          ser.idService === airServiceControl?.idService &&
  ...
  const duplicateCard = (idServiceItem: any, detail: any) => {
    ...
    onDuplicateDetail( airServiceControl?.idControl, airServiceControl?.idService, idServiceItem, newDetail);
  };

  if(airServiceControl) {
     return (    
        ...
        <span key={airServiceControl?.idControl && airServiceControl?.idServiceItem} className={styles.serviceItem}>
          {detail && (
            ...
            >
                ...
                <div className={styles.serviceActions}>
                  ...
                  <div className="flex gap-1">
                    ...
                    <button
                      type="button"
                      className={styles.iconButton}
                      disabled={airServiceControl?.currentIndex === airServiceControl?.serviceDetail?.length - 1}
                      onClick={nextPage}
                    >
                  ...
                  <button
                    ...
                    // disabled={isDisabled}
                    onClick={() => {
                      onRemoveDetail(airServiceControl?.idControl, airServiceControl?.idService, airServiceControl?.idServiceItem, detail.idDetail)
                      prevPage();                
                    }}
                  >
                    <X size={18} />
              ...
              {/* Contenido */}
              <span key={airServiceControl?.currentIndex ? airServiceControl?.currentIndex + 1 : 0} className={styles.serviceItem}>
