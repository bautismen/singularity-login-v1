...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const collection = db.collection("Cat001Incoterms");

    if (method === "GET" && path.endsWith("/catalog-incoterms")) {
      const includeArchived = url.searchParams.get("includeArchived") === "true";
      const filter = includeArchived ? {} : { data_state: 0 };
    ...
    if (method === "GET" && path.match(/\/catalog-incoterms\/[^/]+$/)) {
      ...
      if (!item) {
        return new Response(
          JSON.stringify({ error: "Incoterm not found" }),
          {
            status: 404,
            headers: {
              ...corsHeaders,
    ...
    if (method === "POST" && path.endsWith("/catalog-incoterms")) {
      const body = await req.json();
      console.log("Creating Incoterm:", body);
      ...
      console.log("Incoterm created with ID:", nextId);
    ...
    if (method === "PUT" && path.match(/\/catalog-incoterms\/[^/]+$/)) {
      const id = path.split("/").pop();
      const body = await req.json();
    ...
    if (method === "DELETE" && path.match(/\/catalog-incoterms\/[^/]+$/)) {
      const id = path.split("/").pop();
