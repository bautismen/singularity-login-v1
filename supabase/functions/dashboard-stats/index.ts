...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const quotationData = await quotationView.find({}).toArray();
    ...
    if (quotationData.length > 0) {
      const quotationDoc = quotationData[0];
      quotations = {
        totalCurrentMonth: quotationDoc.totalCurrentMonth || 0,
        statusPercentageCurrentMonth: quotationDoc.statusPercentageCurrentMonth || []
      };
    }
