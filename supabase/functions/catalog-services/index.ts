...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const collection = db.collection("Cat001Services");

    if (method === "GET" && path.endsWith("/catalog-services")) {
    ...
    if (method === "GET" && path.match(/\/catalog-services\/[^/]+$/)) {
    ...
    if (method === "POST" && path.endsWith("/catalog-services")) {
      ...
      const newItem = {
        ...
        email_service_name: body.email_service_name || null,
      ...
      console.log("Service created with ID:", nextId);
    ...
    if (method === "PUT" && path.match(/\/catalog-services\/[^/]+$/)) {
      const id = path.split("/").pop();
      const body = await req.json();
    ...
    if (method === "DELETE" && path.match(/\/catalog-services\/[^/]+$/)) {
      const id = path.split("/").pop();
