...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const collection = db.collection("Cat018RequestTypes");

    if (method === "GET" && path.endsWith("/catalog-request-types")) {
    ...
    if (method === "GET" && path.match(/\/catalog-request-types\/[^/]+$/)) {
      ...
      if (!item) {
        return new Response(
          JSON.stringify({ error: "Request Type not found" }),
          {
            status: 404,
            headers: {
              ...corsHeaders,
    ...
    if (method === "POST" && path.endsWith("/catalog-request-types")) {
      const body = await req.json();
      console.log("Creating Request Type:", body);
      ...
      console.log("Request Type created with ID:", nextId);
    ...
    if (method === "PUT" && path.match(/\/catalog-request-types\/[^/]+$/)) {
      const id = path.split("/").pop();
      const body = await req.json();
    ...
    if (method === "DELETE" && path.match(/\/catalog-request-types\/[^/]+$/)) {
      const id = path.split("/").pop();
